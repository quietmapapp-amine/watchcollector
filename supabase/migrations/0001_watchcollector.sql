...
create table caliber (
  id uuid primary key default gen_random_uuid(),
  name text not null,
  service_interval_months int, -- nullable; can be null if unknown
...
-- Share tokens (public readonly share links)
...
create policy "profile self" on profile
  for select using (auth.uid() = id)
  with check (auth.uid() = id);

create policy "collection self read" on watch_instance
  for select using (user_id = auth.uid()
    or (visibility = 'public')
    or (visibility = 'friends' and exists (
      select 1 from friendship f
      where (f.user_id = user_id and f.friend_id = auth.uid() and f.status='accepted')
         or (f.friend_id = user_id and f.user_id = auth.uid() and f.status='accepted')
    ))
  );

create policy "collection self write" on watch_instance
  for insert with check (user_id = auth.uid())
  using (user_id = auth.uid());

create policy "photo same owner" on photo
  for select using (exists (select 1 from watch_instance w where w.id = photo.watch_instance_id and (w.user_id = auth.uid() or w.visibility in ('public','friends'))))
  with check (exists (select 1 from watch_instance w where w.id = watch_instance_id and w.user_id = auth.uid()));

create policy "service self" on service_event
  for select using (exists (select 1 from watch_instance w where w.id = service_event.watch_instance_id and w.user_id = auth.uid()))
  with check (exists (select 1 from watch_instance w where w.id = watch_instance_id and w.user_id = auth.uid()));

create policy "alert self" on price_alert
  for all using (user_id = auth.uid())
  with check (user_id = auth.uid());

create policy "share token public read" on share_token
  for select using (true); -- Edge function validates token
